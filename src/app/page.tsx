...
export default async function Home() {
  ...
  return <div className="dashboard-shell"><Sidebar /><main className="main-content">
    <header className="topbar"><div><div className="eyebrow">Work Tracker</div><h2>{greeting}, {user.name.split(" ")[0]}</h2><div className="date-label">Here is the shape of your work today.</div></div><div className="user-chip"><div className="avatar">{initials}</div><span>{user.name}</span><SignOutButton /><NewTaskForm projects={projects.map(({ id, name }) => ({ id, name }))} /></div></header>
