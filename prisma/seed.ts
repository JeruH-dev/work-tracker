...
async function main() {
  const user = await prisma.user.upsert({ where: { email: "gideon@example.com" }, update: { passwordHash: await hashPassword("PASSWORD_12345") }, create: { name: "Gideon", email: "gideon@example.com", passwordHash: await hashPassword("PASSWORD_12345") } });
